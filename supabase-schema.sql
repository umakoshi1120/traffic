...
create index if not exists auth_profiles_created_at_idx
  on public.auth_profiles (created_at desc);

alter table public.auth_profiles enable row level security;

drop policy if exists "Anyone can create auth profiles" on public.auth_profiles;
create policy "Anyone can create auth profiles"
...
grant insert on public.auth_profiles to anon;
